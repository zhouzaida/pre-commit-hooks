...
def load_model_zoo(model_index_path: str="model-index.yml"):
    ...
    if not isinstance(model_index_data, dict):
        print(f"Expected the file '{model_index_path}' to contain a dict, but it doesn't.")
        raise ValueError()
...
def check_algorithm(debug: bool, model_index_path: str="model-index.yml") -> int:
    validate_pass = 0
    ...
    try:
        ...
        for collection in collections:
            ...
            if not abstract:
                print(f'Abstract is empty, algorithm name: {display_name}, {readme_path}')
                validate_pass = 1
                break

            if not image:
                print(f'Image is empty, algorithm name: {display_name}, {readme_path}')
                validate_pass = 1
                break
...
def main():
    ...
    return check_algorithm(args.debug)
